...
Answers within 10e-5 of the actual answer will be considered accepted.
...
public class MeanOfArrayAftRemoveElements_LeetCode1619 {
	public static void main(String[] args) {
		...
		int startIdx, endIdx, currentArrayLength, value;
		double mean;
		
		for (int i = 0; i < arr.length; i++) {
			value = 0;
			mean = 0.0;
			currentArrayLength = arr[i].length;
			startIdx = currentArrayLength * 5 / 100;
			endIdx = currentArrayLength - startIdx - 1;
			Arrays.sort(arr[i]);
			for (int j = startIdx; j <= endIdx; j++ ){
				value += arr[i][j];
			}
			
			mean = (double)value / (currentArrayLength - startIdx * 2);
